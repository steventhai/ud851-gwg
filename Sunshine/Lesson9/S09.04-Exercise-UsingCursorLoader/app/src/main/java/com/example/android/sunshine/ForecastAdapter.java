...
//      COMPLETED (7) Generate a weather summary with the date, description, high and low
        ...
        String dateInString = SunshineDateUtils.getFriendlyDateString(mContext, dateInMillis, false);

        int weatherId = mCursor.getInt(MainActivity.WEATHER_ID_INDEX);
        String description = SunshineWeatherUtils.getStringForWeatherCondition(mContext, weatherId);

        double highTemp = mCursor.getDouble(MainActivity.MAX_TEMP_INDEX);
        double lowTemp = mCursor.getDouble(MainActivity.MIN_TEMP_INDEX);

        String highLow = SunshineWeatherUtils.formatHighLows(mContext, highTemp, lowTemp);

        String weatherForThisDay = dateInString + " - " +  description + " - " + highLow;
