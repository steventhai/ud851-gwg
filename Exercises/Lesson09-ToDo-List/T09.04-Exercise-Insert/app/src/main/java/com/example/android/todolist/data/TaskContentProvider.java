...
public class TaskContentProvider extends ContentProvider {
    ...
    public Uri insert(@NonNull Uri uri, ContentValues values) {
        ...
        switch (match) {
            case TASKS:
                long insert = writableDatabase.insert(TaskContract.TaskEntry.TABLE_NAME, null, values);
                if (insert > 0) {
                    returnUri = ContentUris.withAppendedId(TaskContract.TaskEntry.CONTENT_URI, insert);
                } else {
                    throw new SQLException("Failed to insert row into " + uri);
                }
                break;
            default:
                throw new UnsupportedOperationException("Uri is not supported " + uri);
        }
